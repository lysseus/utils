...
;; Defines variables corresponding to the fields specified for
;; struct id assigning the values extracted from struct-expr. 
(define-syntax (define-from-struct stx)
  (syntax-parse stx
    [(_ id struct-expr fld-id ...)
     #`(define-values (#,@(for/list ([x (syntax->list #'(fld-id ...))])
                          (with-syntax ([name x])
                            #`name)))
         (apply values (list #,@(for/list ([x (syntax->list #'(fld-id ...))])
                          (with-syntax ([getter (format-id #'id "~a-~a" #'id x)])
                            #`(getter struct-expr))))))]))

(module+ test
  (test-case "define-from-struct tests"
             (struct foo (w x) #:transparent)
             (struct bar foo (y z) #:transparent)
